...
locals {
  ...
  oidc_id  = replace(local.oidc_url, "https://", "")
}
...
###
# IAM Role for Cortex S3 Access (IRSA)
###
data "aws_iam_policy_document" "cortex_s3" {
  statement {
    effect = "Allow"
    actions = [
      "s3:PutObject",
...
resource "aws_iam_policy" "cortex_s3" {
  name   = "ConduktorCortexS3Policy"
  policy = data.aws_iam_policy_document.cortex_s3.json
}

data "aws_iam_policy_document" "cortex_assume" {
  statement {
    effect  = "Allow"
    actions = ["sts:AssumeRoleWithWebIdentity"]
    principals {
    ...
    condition {
      test     = "StringLike"
      variable = "${local.oidc_id}:sub"
      values   = ["system:serviceaccount:conduktor:*"]
    }
    condition {
      test     = "StringEquals"
      variable = "${local.oidc_id}:aud"
...
resource "aws_iam_role" "cortex" {
  name               = "conduktor-cortex-s3"
  assume_role_policy = data.aws_iam_policy_document.cortex_assume.json
}

resource "aws_iam_role_policy_attachment" "cortex_s3" {
  role       = aws_iam_role.cortex.name
  policy_arn = aws_iam_policy.cortex_s3.arn
}
...
resource "aws_acm_certificate" "wildcard" {
  ...
  certificate_body = tls_self_signed_cert.wildcard.cert_pem
}
